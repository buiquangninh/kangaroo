...
class MassQtyLimit extends MassMultipleUpdate implements \Magento\Framework\App\Action\HttpPostActionInterface
{
    ...
    public function processAndSaveData($postData)
    {
        ...
        foreach ($collection as $appliedProductsModel) {
            if ($appliedProductsModel->getFlashSalesId() == $postData['flashsales_id']) {
                $appliedProductsModel->setData('qty_limit', 0)->save();
            }
        }
