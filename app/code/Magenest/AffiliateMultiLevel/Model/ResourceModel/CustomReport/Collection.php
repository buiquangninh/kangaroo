 ...
 */
namespace Magenest\AffiliateMultiLevel\Model\ResourceModel\CustomReport;
...
class Collection extends \Magento\Framework\Data\Collection
{
            ...
            while ($dateStart <= $dateEnd) {
                switch ($this->_period) {
                    ...
                    case 'year':
                        ...
                        break;
                    case 'quarterly':
                        $interval = $this->_getQuarterlyInterval($dateStart, $dateEnd, $firstInterval);
                        $firstInterval = false;
                        break;
                    default:
                        break 2;
    ...
    protected function _getQuarterlyInterval(\DateTime $dateStart, \DateTime $dateEnd, $firstInterval)
    {
        $interval = [];

        if ($dateStart->diff($dateEnd)->m <= 3) {
            $interval['period'] = "01";
            $interval['start'] = $this->_localeDate->convertConfigTimeToUtc($dateStart->format('Y-01-01 00:00:00'));
            $interval['end'] = $this->_localeDate->convertConfigTimeToUtc(
                $dateStart->setDate(
                    $dateStart->format('Y'),
                    $dateStart->format('m'),
        ...
        } elseif($dateStart->diff($dateEnd)->m <= 6 && $dateStart->diff($dateEnd)->m > 3) {
            $interval['period'] = "02";
            $interval['start'] = $this->_localeDate->convertConfigTimeToUtc($dateStart->format('Y-04-01 00:00:00'));
            $interval['end'] = $this->_localeDate->convertConfigTimeToUtc(
                $dateStart->setDate(
                    $dateStart->format('Y'),
                    $dateStart->format('m'),
        ...
        } elseif($dateStart->diff($dateEnd)->m <= 9 && $dateStart->diff($dateEnd)->m > 6) {
            $interval['period'] = "03";
            $interval['start'] = $this->_localeDate->convertConfigTimeToUtc($dateStart->format('Y-07-01 00:00:00'));
            $interval['end'] = $this->_localeDate->convertConfigTimeToUtc(
                $dateStart->setDate(
                    $dateStart->format('Y'),
                    $dateStart->format('m'),
        ...
        } elseif($dateStart->diff($dateEnd)->m <= 12 && $dateStart->diff($dateEnd)->m > 9) {
            $interval['period'] = "04";
            $interval['start'] = $this->_localeDate->convertConfigTimeToUtc($dateStart->format('Y-10-01 00:00:00'));
            $interval['end'] = $this->_localeDate->convertConfigTimeToUtc(
                $dateStart->setDate(
                    $dateStart->format('Y'),
                    $dateStart->format('m'),
    ...
    {
        return ['day' => __('Day'), 'month' => __('Month'), 'year' => __('Year'), 'quarterly' => __('Quarterly')];
    }
