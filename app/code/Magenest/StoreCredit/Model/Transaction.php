...
class Transaction extends AbstractModel implements IdentityInterface, TransactionInterface
{
    ...
    public function createTransaction($code, $customer, $actionObject)
    {
        $queueData = [
            ...
            'action_object' => $actionObject->getData()
        ];

        $this->publisher->publish('storecredit.update', $this->serializer->serialize($queueData));
    }
     ...
     */
    public function processTransaction($code, $customer, $actionObject)
    {
        ...
        if (!$this->_registry->registry(
            CreateStoreCreditTransactionFromAffiliate::IGNORE_EVENT_TRANSACTION_STORE_CREDIT
        )) {
