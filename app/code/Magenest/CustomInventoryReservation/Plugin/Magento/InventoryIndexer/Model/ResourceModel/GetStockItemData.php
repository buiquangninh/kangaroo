            ...
            try {
                if ($this->defaultStockProvider->getId() === $stockId) {
                    ...
                    $selectInventoryStock->from(
                        ...
                        [
                            GetStockItemDataInterface::QUANTITY => new \Zend_Db_Expr('SUM(' . IndexStructure::QUANTITY . ')'),
                            GetStockItemDataInterface::IS_SALABLE => IndexStructure::IS_SALABLE,
                            'area_code'
                        ]
                    )->where(
                        IndexStructure::SKU . " = '$sku' AND area_code = '{$areaCode}' AND is_salable = 1"
                    );
        ...
        try {
            if ($this->defaultStockProvider->getId() === $stockId) {
                ...
                $select = $connection->select()->from(
                    ...
                    IndexStructure::SKU . " = '$sku' AND area_code = '{$areaCode}'"
                );
                $qty = $connection->fetchRow($select);
            }
