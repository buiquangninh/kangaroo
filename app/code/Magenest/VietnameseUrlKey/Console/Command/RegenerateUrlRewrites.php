...
class RegenerateUrlRewrites extends RegenerateUrlRewritesAbstract
{
    ...
    {
        $this->setName('catalog:urlrewrites:regenerate')
            ...
            ->setDefinition([
                ...
                new InputOption(
                    self::INPUT_KEY_REINDEX,
                    null,
                    InputOption::VALUE_NONE,
                    'Run reindex when URL rewrites are generated.'
                ),
                new InputOption(
                ...
                new InputOption(
                    self::INPUT_KEY_CACHE_FLUSH,
                    null,
                    InputOption::VALUE_NONE,
                    'Run cache:flush when URL rewrites are generated.'
                ),
                new InputOption(
                    self::INPUT_KEY_CACHE_CLEAN,
                    null,
                    InputOption::VALUE_NONE,
                    'Run cache:clean when URL rewrites are generated.'
                ),
                new InputOption(
                ...
                new InputOption(
                    self::INPUT_KEY_PRODUCT_SKU,
                    null,
                    InputArgument::OPTIONAL,
                    'Specific products SKU, e.g.: \'WSH12\''
                )
            ]);
    }
    ...
    public function getCommandOptions()
    {
        ...
        if (isset($options[self::INPUT_KEY_REINDEX]) && $options[self::INPUT_KEY_REINDEX] === true) {
        ...
        if (isset($options[self::INPUT_KEY_CACHE_CLEAN]) && $options[self::INPUT_KEY_CACHE_CLEAN] === true) {
            $this->_commandOptions['runCacheClean'] = true;
        }

        if (isset($options[self::INPUT_KEY_CACHE_FLUSH]) && $options[self::INPUT_KEY_CACHE_FLUSH] === true) {
        ...
        if (isset($options[self::INPUT_KEY_PRODUCT_SKU])) {
            $this->_commandOptions['productSku'] = $options[self::INPUT_KEY_PRODUCT_SKU];
            $distinctOptionsUsed++;
        }
        ...
        if (
            ...
            $this->_errors[] = __(
                ...
                [
                    'o1' => self::INPUT_KEY_REGENERATE_ENTITY_TYPE_PRODUCT,
                    'o2' => self::INPUT_KEY_CATEGORIES_RANGE,
        ...
        if (
            ...
            $this->_errors[] = __(
                "ERROR: you can not use this options together (logical conflict):\n'--%o1' with '--%o2'/'--%o3'/'--%o4'",
                [
                    'o1' => self::INPUT_KEY_REGENERATE_ENTITY_TYPE_CATEGORY,
                    'o2' => self::INPUT_KEY_PRODUCT_ID,
                    'o3' => self::INPUT_KEY_PRODUCTS_RANGE,
                    'o4' => self::INPUT_KEY_PRODUCT_SKU
                ]
            );
        }
