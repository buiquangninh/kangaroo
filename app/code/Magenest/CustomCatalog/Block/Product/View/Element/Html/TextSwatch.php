...
namespace Magenest\CustomCatalog\Block\Product\View\Element\Html;

use Magento\Framework\View\Element\AbstractBlock;

class TextSwatch extends AbstractBlock
{
    /**
     * Options
     *
    ...
    protected function _toHtml()
    {
        ...
        $id = $this->getId();
        $isArrayOption = true;
        $html = "<div name='{$this->getName()}' id='{$id}-option-wrapper' tabindex=\"0\" role=\"listbox\">";
        foreach ($this->getOptions() as $key => $option) {
            ...
            if($value) {
                $html .= "<div class=\"custom-swatch-option swatch-option text\" id=\"option-label-{$value}\" index=\"{$key}\" tabindex=\"{$key}\" role=\"option\" data-option-value='{$value}'>{$label}</div>";
            }
        ...
        $html .= '</select>';
        $html .= "<script>require(['jquery'], function ($) {
             $(document).ready(function () {
                $('div#{$id}-option-wrapper .custom-swatch-option').on('click', function () {
