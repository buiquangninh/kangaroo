define([
    ...
    $.widget('mage.createVideoPlayer', {
         ...
         */
        reset: function () {
            if(this.element.find(this.options.container).children('video')) {
                this.element.find(this.options.container).empty();
                this.element.find(this.options.container).removeClass('no-before');
                this.element.find(this.options.qqUploadList).empty();
            }
    ...
    $.widget('mage.updateInputFields', {
        options: {
            reset: false,
            DOM: {
                urlField: 'input[name="video_path"]',
    ...
    $.widget('mage.newVideoDialog', {
        ...
        _pathVideo: '/media/catalog/product/video/',

        _pathVideoTmp: '/media/catalog/tmp/product/video/',
        ...
        _videoInformationGetPathField: 'null',
        ...
        _bind: function () {
            ...
            this._videoUrlWidget = this.element.find(this._videoPathSelector).videoData({
            ...
            this._videoInformationGetUrlField = this.element.find(this._videoPathSelector);
        ...
        _onGetVideoInformationClick: function () {
            ...
            if (this.element.find(this._videoPathSelector).valid()) {
        ...
        _onImageLoaded: function (result, file, oldFile, callback) {
            ...
            data['video_url'] = this.element.find(this._videoPathSelector).val();
        ...
        _create: function () {
            ...
            this.element.modal({
                ...
                opened: function () {
                    ...
                    widget.element.find('#video_path').click();
        ...
        isValid: function (callback) {
            ...
            if (!videoData || (!videoData.file && !videoData.video_url)) {
                videoForm.validation();
            }
         ...
         */
        _onUpdate: function () {
            var inputFile, videoPath, itemId, _inputSelector, mediaFields, imageData, flagChecked, fileName, callback;

            this.isValid($.proxy(
                function () {
                    imageData = this.imageData || {};
                    inputFile = this.element.find(this._videoPreviewInputSelector);
                    videoPath = this.element.find(this._videoPathSelector);
                    ...
                    if(videoPath.val()) {
                        imageData.video_url = videoPath.val();
                        this._gallery.find('input[name*="' + itemId + '][video_url]"]').val(imageData.video_url);
                    }
        ...
        _onOpenDialog: function (e, imageData) {
            ...
            if (imageData['media_type'] === 'external-video') {
                ...
                if(imageData.video_url === imageData.video_url_default) {
                    modal.find('.video-player-container').html('<video id="product-video" autoplay muted loop width="100%" height="100%" playsinline>' +
                        '<source src="' + window.location.origin + this._pathVideo + imageData.video_url + '"></video>');
                    modal.find('.video-player-container').addClass('no-before');
                } else if (imageData.video_url && imageData.video_url !== imageData.video_url_default) {
                    modal.find('.video-player-container').html('<video id="product-video" autoplay muted loop width="100%" height="100%" playsinline>' +
                        '<source src="' + window.location.origin + this._pathVideoTmp + imageData.video_url + '"></video>');
                    modal.find('.video-player-container').addClass('no-before');
                }
