...
class Helper extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function __construct(
        ...
        $this->customerSegmentRepository= $customerRepository;
    ...
    public function sendNotificationInMagento($notification)
    {
        ...
        $listCustomerSegment = $this->json->unserialize($notification['customer_segment']);
        ...
        while ($currentPage <= $totalPage){
            ...
            foreach ($CustomerPart as $customer) {
                ...
                $currentCustomerSegments = $this->getCustomerSegmentIds($customer['email'], $customer['website_id']);
                //remove notification if not meet conditions
                if (!in_array('0', $listStore) && !in_array($customerStoreId, $listStore)) {
                    continue;
                }
                if (!in_array($customerGroupId, $listCustomerGroup) && !array_intersect($currentCustomerSegments, $listCustomerSegment)) {
                    continue;
                }
