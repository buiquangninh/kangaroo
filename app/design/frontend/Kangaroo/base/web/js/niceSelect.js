...
!function(e, _) {
    e.fn.niceSelect = function(t) {
        function s(t) {
            t.after(e("<div></div>").addClass("nice-select").addClass(t.attr("class") || "").addClass(t.attr("disabled") ? "disabled" : "").attr("tabindex", t.attr("disabled") ? null : "0").html('<span class="current"></span><ul class="list"><div class="select-search-wrapper"><input class="input-text option-search" type="text"/><button type="button" class="search-address-button"></button></div></ul>'));
            var s = t.next()
                , n = t.find("option")
            ...
            s.find(".select-search-wrapper .option-search").on('keyup', _.debounce(function () {
                    var searchInput = e(this).val().toLowerCase();

                    if (!searchInput) {
                        s.find('li').show();
                    } else {
                        s.find('li').each(function (index, ele) {
                            var innerText = e(ele).text();
